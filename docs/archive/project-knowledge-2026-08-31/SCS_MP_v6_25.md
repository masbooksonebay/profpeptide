...
**Version:** v6.25 (August 10, 2026) — adds the **Reddit community attempt** (banned in 3 hours, appeal route non-existent, rules from it) alongside v6.24's six standing rules, the verify-the-premise rule, and the Claude Design unsent-attachment behavior.
...
## 🔴 THE REDDIT COMMUNITY ATTEMPT — banned in three hours (August 10, 2026)

**r/ProfPeptide was created at 9:17 AM and banned by roughly 12:15 PM.** Recorded in full because the lesson is cheap here and expensive elsewhere.

**What was built:** a vendor-review community with six rules, a description, post guidelines, a disclosure widget, an icon, correct discoverability settings, and two posts — a welcome post explaining the community, and a codes post listing three affiliate codes with disclosure.

**The sequence:** welcome post removed by Reddit with **no notification and no mod-log entry**, then the subreddit banned. **No message was ever sent for either action**, and the ban page cites no rule. Reddit's own appeals policy states the account is notified of the removal reason and given appeal instructions. That didn't happen.

**Most likely cause:** a three-hour-old subreddit whose second post was an affiliate code list is close to a textbook spam signature. The escalation pattern — automated removal, then ban — fits a classifier, not a human review of the disclosure language.

### 🔴 The appeal route does not exist

Four official paths, all circular:
1. r/ModSupport modmail — **returns "You can't message that user."** `u/ModSupport`, the placeholder user account, is itself `is_suspended: true`; the legacy compose that understood `/r/subreddit` recipients was deprecated, so the unified form resolves it as a username and errors
2. The Help Center appeal form — **removes its own email, subject, details fields and Submit button** on selecting the banned-community option, replacing them with a callout linking back to the broken modmail URL
3. r/redditrequest — points at both of the above; also ineligible (content bans excluded, 30-day bar, 90-day/100-karma requirement)
4. An r/bugs post reporting the routing defect — **removed**

**The Moderator Code of Conduct form works**, but it's the wrong desk — CoC actions come from `u/ModCodeofConduct` with a notice, and the form asks which CoC action you're appealing. There was none.

### What it cost, and what it didn't

**🟢 The account is clean** — `is_suspended: false`, `can_create_subreddit: true`, no restriction notice. This stopped at the subreddit.
**🟢 No Google impact** — zero results three ways; the sub never entered the index in three hours.
**⚠️ Domain filtering: UNVERIFIED.** profpeptide.com never appeared in a link post, only inside self-post body text, so empty search results prove nothing.
**Lost:** the codes post body was never captured before the permalinks went dark. The welcome post was captured ~10 minutes before.

### Rules from this

**⚠️ DO NOT create a replacement subreddit.** That's ban evasion, and it escalates a subreddit ban into an account action.

**⚠️ Stop after one removal.** Repeated posts after a removal is the pattern that escalates. One removed post is noise; two is a pattern.

**🔒 If a community is ever attempted again: no affiliate codes, ever.** The review function was viable. The codes were the trigger, and disclosure doesn't change what a post does.

**🟢 The salvage:** the welcome post's framing — who runs the site, what assessment involves, what the affiliate relationship does and doesn't change — was good E-E-A-T content sitting on a platform that deleted it in three hours. **It now lives on `/methodology`**, where nobody can remove it.

> **The wider lesson:** PP's revenue comes from X and code attribution, neither of which touched Reddit. Before building on a platform, ask what happens when it removes you — and whether the same content could live somewhere you own.
