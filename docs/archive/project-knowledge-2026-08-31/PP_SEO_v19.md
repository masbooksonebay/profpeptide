...
## 🔴 SCOPE WARNING (added July 16, end of session) — READ BEFORE ACTING ON THIS DOC

**This document analyses ONE channel: search → PP page → vendor link. That may not be where the revenue is.**

First-party commission data (PP_COMMISSIONS **v4**) shows the correlation between search impressions and commission is **INVERTED**:

| Vendor | Commission | GSC impr | $ / impression |
|---|---|---|---|
| **Peptide Partners** | **$4,283.94** (~51% of all revenue) | **96** | **$44.62** |
| Amino Club | $659.46 | **4,371** | **$0.15** |

**PP's commission is code-based.** A buyer sees `PROFPEPTIDE` on X, goes straight to the vendor, uses the code — **PP is paid and the buyer never touches profpeptide.com.** No impression, no click, invisible to GSC. Peptide Partners' dashboard shows **256 lifetime visits → 120 referrals (46.88%)**, and a recent period **above 100%** — mathematically only possible if the code is used without clicking the link.

**Nothing in this doc is wrong for what it measures.** But:
- **Do NOT treat search rank or CTR as a revenue proxy.**
- **The "Ascension anomaly" below is NOT an anomaly** — pos 5.3 / 12 impressions / real sales. It sells through code. Disregard that section's framing.
- The priority plan optimises the link channel. Weigh it against the code channel before spending a month on it.

**→ Read PP_COMMISSIONS v4 first. The open question there (the AffiliateWP referring-URL log) is worth more than anything in this document.**
