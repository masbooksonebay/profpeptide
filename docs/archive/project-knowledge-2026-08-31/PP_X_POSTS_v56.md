...
| Change | Detail |
|---|---|
| 🔒 **CODES RENDER UPPERCASE — always** | PP renders every vendor code in UPPERCASE regardless of how the affiliate portal displays it. GoAffPro showed `profpeptide` lowercase for OMP and Athena; both shipped lowercase and were corrected (`22fb1b6`). ⚠️ The reason this matters beyond consistency: `check:codes`' CODE_RE is **uppercase-anchored**, so a lowercase code is invisible to all three guard tiers. Uppercasing them raised the guard count 145 → 152 (+7 literals that had been unguarded). **A lowercase code is an unguarded code.** |
| 🔴 **Peptides.gg consolidated → PROFPEPTIDE** | Vendor-initiated, Aug 28 (`b6137d0`). Sixth exhibit in the code-drift class and the **second consolidation in two days** — reframes the portal-check list from a defect hunt into a consolidation campaign with a 2-for-2 hit rate. ⚠️ The sweep was surgical: PROF15 is ALSO purity-peptides' live code, and `?ref=PROF15` at aero-peptides is a GoAffPro **affiliate identifier, not a coupon** — a blind global replace would have broken two live vendors. **Always inventory every occurrence WITH its owning vendor before changing one.** |
| 🔒 **Swiss Chems stays PROF10** | A portal setting labelled "change your code" changed the **referral LINK**, not the code. Mark reverted the link to `?ref=prof10` to match the repo. **Lesson: verify which field actually moved before touching the repo.** Remaining non-standard codes: `PROF10` behemoth/purerawz/swiss-chems · `PROF15` purity-peptides · `PROF20` vital-core. |
| 🆕 **Crush Research §7 decode + presale** | Triple Agonist = Retatrutide, Double Agonist = Tirzepatide — **confirmed on PP's own coupon page**, not inferred. Final presale opened **Fri Aug 28, 5:00 PM MT**, no published end date, max 2/customer. ⚠️ Presale prices are the vendor's own offer on **specific SKUs** — NOT a sitewide sale, so §3's always-stacks rule does NOT apply; stacking is unconfirmed and no post claims it. |
| 🆕 **Capstone category set (§7b)** | Seven categories drafted: metabolic · tissue · secretagogue · immune · dermal · neuro. **Vascular deliberately not drafted** — PT-141 is the only compound in it, so line 3 has nothing to name (the thin-category rule). ⚠️ Link path `/shop/` vs `/catalog/` unconfirmed — v55 recorded `/catalog/`; the live store breadcrumbs as "Home / Shop". |
| 🆕 **§9 news posts — the first real one** | The Lilly-lawsuits article produced PP's first substantive news post set (X + Facebook + Instagram carousel). Pattern recorded in §9 below. |
| 🆕 **POSTED LOG** | Royal ×3 and Ignite ×3 **posted Aug 26** — first sets after the code correction. |
| ⚠️ **Still owed from part 5** | The "one-URL rule" remains uncaptured — Mark to restate; do not invent it. |
...
## 🔒 9b. NEWS POSTS ACROSS SURFACES (new, Aug 29)

A news story now produces up to three artifacts. They are **not** the same text.

| Surface | Shape | Link behavior |
|---|---|---|
| **X** | 3–4 short paragraphs; open with the surprise, not the summary. No code, no vendor, no urgency. | Link is live. ⚠️ Linking `/news` renders the generic hub OG card; linking the **article** renders its own card matching the post's subject. Choose deliberately. |
| **Facebook** | Longer and self-contained — people read in-feed and click only if the text earned it. | Link is live; the OG card builds in the composer. Watch it render before posting. |
| **Instagram** | **A 5-slide carousel** — IG allows no links, so the story must carry itself in the images. See PP_SOCIAL. | No link. The URL is **read, not tapped** — keep it short and typeable, and make it the CTA element rather than a footer line. |

🔒 **THE INVARIANT ACROSS ALL THREE:** a lawsuit is an **allegation, not a finding**. That sentence survives every revision on every surface. For litigation stories, give it its own slide in the carousel.

🔒 **PP never refers to itself, its roster, or its listings in news content.** No "one of our vendors," no "vendors we list." The site reports the industry; it is not a character in the story. (⚠️ The Aug 17 three-lanes article predates this rule and contains self-reference — left deliberately, since that passage is doing honest disclosure about where PP sits.)
...
| Amino X | PROF15 | 15% |
| Oasis Labs | PROFPEPTIDE | 15% — *changed from PROF15, Aug 10*; ⚠️ Mark's portal answer still outstanding |
| **Peptides.gg** | **PROFPEPTIDE** | 15% — 🔴 consolidated from PROF15 by the vendor, Aug 28 (`b6137d0`) |
| **Order My Peptides** | **PROFPEPTIDE** | 15% customer / 20% commission — uppercase per the standing rule |
| **Athena Peptides** | **PROFPEPTIDE** | **10% customer** (15% is COMMISSION — §1b-3) |
| **Crush Research** | PROFPEPTIDE | 10% |
