...
## ✅ THE GITHUB TEMPLATE SYSTEM — COMPLETE AND PROVEN (July 17, 2026)

**Both templates are on GitHub. Both were tested from a FRESH CD chat. Both worked. Chat continuity is no longer a dependency for HR social.**

| Artifact | Files | GitHub path (`masbooksonebay/hybrid-rockstar-site`, `main`) |
|---|---|---|
| **Carousel** | 1 self-contained file | `social/HR_CAROUSEL_TEMPLATE.dc.html` (+ `social/support.js`) |
| **Reel** | **4 files — must travel together** | `social/HR_REEL_TEMPLATE.dc.html` · `social/HR_REEL_TEMPLATE_scene.jsx` · `social/HR_REEL_TEMPLATE_assets.js` · `social/animations.jsx` |

**Commits:** `d7f8c45` (carousel) · `4e0a930` (support.js) · `af46d67` (4 reel files).

### 🔑 A REEL IS NOT A SINGLE FILE
Unlike the carousel, the reel wrapper **x-imports three siblings**:
```html
...
```
- `HR_REEL_TEMPLATE.dc.html` — wrapper (Barlow `<link>` + build-method comment)
- `HR_REEL_TEMPLATE_scene.jsx` — the 4 scenes (`SceneHook` / `SceneWorkout` / `SceneCue` / `SceneCTA`). **Edit scenes 1–3; scene 4 is fixed.**
- `HR_REEL_TEMPLATE_assets.js` — the hook photo as `window.HR_REEL_BW_IMG` (ships with a placeholder stub)
- `animations.jsx` — **the SHARED timeline engine.** Required by ANY reel; permanent repo fixture, not template-specific.

**Paths are `./`-relative, not project-root absolute** — so they resolve correctly from `social/`. **No rewrite needed.** (CC verified this rather than assuming; the template's own comment says "at project root," which is misleading — the real mechanism is "beside the .dc.html.")

### Per-day workflow (any fresh chat, no history required)
1. **Carousel** → point CD at `social/HR_CAROUSEL_TEMPLATE.dc.html`, fill the `[BRACKETED]` tokens.
2. **Reel** → point CD at all **four** reel files, edit `[BRACKETED]` tokens in `_scene.jsx` + the photo in `_assets.js`.
3. **Always open with a quote-back check** (subheadline markup / scene count / Scene 4 wordmark). If CD can't quote from a file it just read, it isn't reading it.
4. **Mark checks the delivered file.** CD's self-verification is worthless — 4 false "verified by looking at the pixels" claims in one session.

---

## 🔧 WORDMARK COLOUR — CORRECTED FROM THE ARTIFACTS (July 17)

| Asset / slide | "HYBRID" | "ROCKSTAR" | Source |
|---|---|---|---|
| **Carousel — intro/photo slide (1)** | **YELLOW** | **YELLOW** | shipped IG grid |
| Carousel — cover (2) | WHITE | YELLOW | shipped IG grid |
| Carousel — app-pitch (last) | WHITE | YELLOW | shipped IG grid |
| **Reel — hook scene (1)** | **WHITE** | YELLOW | W3D1 markup + shipped IG grid |
| **Reel — CTA scene (4)** | **YELLOW** | **YELLOW** | 🔧 **W3D1 markup, read by CD + confirmed by CC** |

**🔧 v10's reel-CTA row was WRONG — I inferred it.** The actual W3D1 markup:
```jsx
React.createElement('div', { style: { …, fontWeight: 700, fontSize: 44,
  letterSpacing: '0.42em', color: YELLOW, … } }, 'Hybrid Rockstar')
```
**A single `'Hybrid Rockstar'` string, `color: YELLOW` — both words.** The hook scene is two separate divs (`'Hybrid'` at `#fff`, `'Rockstar'` at `YELLOW`). **Only the hook splits the colours.**

> **This is the THIRD doc-vs-artifact miss in two days.** Intro subheadline (doc said "FULL 12-WEEK HYROX PROGRAM", markup says `Full HYROX<br>Training Program`) · wordmark weight (doc said Bold 700, markup says **600**) · reel CTA (doc said white+yellow, markup says **both yellow**). **When a doc and an artifact disagree, the artifact wins. Read the markup.**
>
> **The templates are now the source of truth for typography and colour — not this doc's description of them.**

**Good process note:** CD was *instructed* "white+yellow on both scenes," read W3D1's markup, found otherwise, spliced scene 4 byte-for-byte anyway, and flagged the conflict. **That's the correct behaviour** — the instruction was wrong and the artifact won.

---

## ⚠️ TWO CLEANUPS OUTSTANDING (neither blocks a build)

**1. The reel template's build-method comment contradicts itself.** It reads: *"CTA scene → both words YELLOW (This differs from the CAROUSEL intro slide, where both words are yellow.)"* — the parenthetical belongs on the **hook** line, not the CTA line. The markup is unambiguous so builds are safe, **but the template is now the source of truth and must not contain a self-contradicting note.** That is exactly the class of error that cost a full day. Fix on the next CD reel session → CC re-commits.

**2. W3D2 is the shipped outlier on the intro subheadline.** From the live IG grid:
- **W3D2** → "FULL **12-WEEK** HYROX PROGRAM"
- **W3D3 / W3D4 / W3D5** → "FULL HYROX TRAINING PROGRAM"

W3D2 is the only one using the 12-WEEK phrasing — **very likely the source of v7's wrong "built record."** The template now locks `Full HYROX<br>Training Program`, so everything from here is consistent. **Mark's call whether one live post is worth correcting.**
...
- **W3D5** · IG · carousel + reel · **Engine** · **form slide: Rowing** · built July 17 — **first day built entirely from the GitHub templates in fresh CD chats.** Cover subline "(Steady State + Z3 Efforts)" — from the stimulus's 2nd phrase, since "Engine" has no app parenthetical and the stimulus's opening phrase ("Engine peak") is redundant under the headline.
