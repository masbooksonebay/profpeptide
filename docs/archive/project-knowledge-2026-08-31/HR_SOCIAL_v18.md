...
**Status:** ACTIVE. Full replacement, supersedes v17 and v16.
**Date:** August 23, 2026
...
## 12. STATE — August 23, 2026

**Posted to Instagram:** through **W8D5** (per Mark — ⚠️ the feed itself was NOT checked; Instagram is authenticated and Claude in chat cannot reach it).

**Next session: W8D6 — Priority Station Half-Sim, race-order stations 5–8** (Row → Farmers Carry → Sandbag Lunges → Wall Balls).

⚠️ **W8D6 is a SIMULATION day — two rules bite:**
- Hashtag is **`#hybridracing`**, never `#racesimulation`
- **FullRox is *strongly recommended*; HalfRox is a FALLBACK ONLY** — the program says so explicitly. Do not present them as equal options (§5 exception).

⚠️ **W7D4 carousel published without the `[ PEAK ]` eyebrow.** Still unresolved — Mark to compare covers and decide on deletion.
