...
**Status:** ACTIVE. Full replacement, supersedes v18 and earlier.
**Date:** August 26, 2026
...
| Change | Detail |
|---|---|
| 🔴 **The templates now carry the photo-prep contract (§3)** | Committed at `951f210` after the W8D6 failure: monochrome (pure grayscale R=G=B, pixel-verified against the shipped W3D1 asset), prep ORDER (mono first, THEN resize — oversized base64 silently drops the webfont), verbatim placeholder replacement, and a stop-don't-deliver acceptance check. These rules previously lived ONLY in one CD chat's context — failure #7. |
| 🆕 **Two conventions still template-homeless (§3)** | Minimum source resolution and export structure/delivery scope — queued for the next CC repo pass. Until then, prompts carry them. |
| 🆕 **The prompt-drafting rule (§0)** | Locked-format prompts get the FULL doc read before drafting, regardless of how the request is phrased. W8D6 broke partly because a CD prompt was drafted from session memory when the request was batched. |
| **§12 STATE** | Posted through W8D6 (pending Mark's post). Next: W9D1, APEX block week 3. |
...
| Change | Detail |
|---|---|
| 🔴 **Reel filenames corrected (§2)** | v17 said `_scene.jsx` / `_assets.js`. Both wrong — CD's own x-import reads `HR_REEL_TEMPLATE_scene.jsx` / `HR_REEL_TEMPLATE_assets.js`. Prompts written from the doc pointed at files that don't exist. |
| 🆕 **The source photo is documented (§3)** | Mark attaches it to the CD chat's library. Never stated anywhere before; a build stalled on it 2026-08-23. |
| 🆕 **Build order is documented (§3)** | One CD chat, carousel first, then the reel into the same chat. Includes what to do when CD's context gets trimmed. |
| **§12 STATE updated** | Posted through W8D5. Next is W8D6 Half-Sim — a simulation day, so the `#hybridracing` and FullRox-recommended rules both bite. |
...
| **Reel** | `social/HR_REEL_TEMPLATE.dc.html` (wrapper) + `social/HR_REEL_TEMPLATE_scene.jsx` + `social/HR_REEL_TEMPLATE_assets.js` + `social/animations.jsx` |

All four reel files are required — the wrapper x-imports the other three with `./`-relative paths, so they must sit side by side.

🔴 **FILENAME CORRECTION (2026-08-23).** v17 and the process supplement both listed the scene and assets files as `_scene.jsx` and `_assets.js`. **Both were wrong.** CD's quote-back of the wrapper's own x-import line reads:

```
<x-import component-from-global-scope="ReelSceneTemplate"
  from="./animations.jsx ./HR_REEL_TEMPLATE_assets.js ./HR_REEL_TEMPLATE_scene.jsx"
  hint-size="1080px,1920px"></x-import>
```

The correct names are **`HR_REEL_TEMPLATE_scene.jsx`** and **`HR_REEL_TEMPLATE_assets.js`**. The error survived because the quote-back happens *after* CD has already located the files, so a prompt naming them wrongly still produced a correct build. Prompts written from the doc pointed at files that don't exist.
...
- **Reel:** the Scene 4 CTA wordmark element from `HR_REEL_TEMPLATE_scene.jsx`, and the x-import line from the wrapper.
...
### 🔒 The source photo — Mark supplies it

**Mark attaches the session photo to the CD chat's library before the build.** CD uses whatever is attached. There is no repo photo source, no stock library, and CD does not generate one.

⚠️ **A carousel prompt without an attached photo cannot be built.** Recorded 2026-08-23 — the W8D5 reel prompt was sent to a CD project with no photo and CD stopped to ask for one. Neither this doc nor the process supplement had ever said where the photo came from.

### 🔒 The photo-prep contract now lives IN THE TEMPLATES — commit `951f210`

After W8D6 (2026-08-25/26): the monochrome treatment existed in NO written source — not the template, not this doc, not the process doc. It lived only in the original CD chat's accumulated context, and when a fresh chat (and then the decayed old chat) built without it, both produced color intros. CC's diagnosis confirmed: zero commits to social/ since the last correct build; the rule simply had no home.

**The fix (committed and pushed at `951f210`, render-equivalence proven byte-identical):** the carousel header, reel wrapper, and reel assets header now state the full contract —
1. **Monochrome FIRST** — pure grayscale (R=G=B; pixel-verified: 182 samples off the shipped W3D1 asset, zero channel deviation). Never duotone, never the color source.
2. **THEN resize** to 1080×1350 JPEG ~200KB. Order matters: oversized base64 trips the export serializer → canvas fallback → SILENT loss of the webfont.
3. **Replace the Slide 1 placeholder VERBATIM** — object-fit:cover, object-position:center — only the src changes.
4. **Acceptance:** a non-monochrome rendered Slide 1 means stop and report, never deliver.
The reel's `HR_REEL_BW_IMG` is the SAME prepped JPEG the day's carousel used.

⚠️ **Two conventions surfaced by the same arc are NOT yet in the templates** (queued for CC):
- **Minimum source resolution:** a 387×516 source forced a 2.79× upscale — soft slide. Sources must be ≥1080×1350; upscaling should be a stop-and-report condition.
- **Export structure & delivery scope:** slides land in `exports/W#D#/`, named per the prior day's folder; the reel MP4 is named `HR_W#D#_REEL.mp4` on save (the export won't name it); delivery is the day's folder/file ONLY, never the whole exports directory. A fresh session delivered loose numbered files and an archive zip because this convention had no written home either.

Until both land in the templates, **every CD prompt states them explicitly.**
...
### 🔒 Build order — carousel first, then reel

**Both assets are built in ONE CD chat, carousel first.** When the carousel finishes, Mark sends the reel prompt into the same chat.

**Why the order is fixed:** the reel's Scene 1 reuses the carousel intro's already-resized 1080×1350 JPEG. Building the reel first means resizing a photo the carousel must then match.

⚠️ **CD's context can be trimmed mid-session.** On 2026-08-23 CD reported the W8D5 carousel "was in an earlier, now-trimmed part of this session" and could not see it. If that happens, the carousel is rebuilt before the reel — the reel cannot inherit a photo CD can no longer see.
...
## 12. STATE — August 26, 2026

**Posted to Instagram:** through **W8D5**; **W8D6 built and verified** (carousel five slides + 20.0s reel), posting on Mark's side. ⚠️ The feed itself was NOT checked; Claude in chat cannot reach it.

**Next session: W9D1 — Week 9 is APEX, peak block week 3.** Read the week header in HR_PROGRAM_EXPORT before drafting; the block tag stays `[ PEAK ]`.

**The W8D6 build cycle (2026-08-25/26) — what it established:**
- The templates are the build contract; CD executes them. A correct build is now possible from the files alone with zero prior chat context — that was the point of `951f210`.
- The reel ran 20.0s against the ~13–16s target because Scene 2 carries all six session rows — the complete-session rule outranks the duration target, by design.
- Template-locked type sizes outrank copy: the "@ race weight" annotation was dropped from Scene 2 rows (condition preserved in Scene 3's opening words) rather than editing layoutRows. **Adjust the copy, never the template** — both surfaces.

**Outstanding:**
- **W7D4 carousel** published without the `[ PEAK ]` eyebrow — Mark to compare covers and decide on deletion (open since Aug 11)
- Reel template's self-contradicting build-method comment — batch with the two template-homeless conventions above in one CC pass
- **Image fit** — resolved in practice by `951f210`'s verbatim-placeholder rule (object-fit:cover); the §1b OPEN flag can be considered closed
