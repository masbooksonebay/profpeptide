# SCS_REF_INDEX v18
**Supersedes v16. Updated 2026-08-26.**
...
| Doc | Version | Covers |
|---|---|---|
| **PP_X_POSTS** | **v54 ← NEW (full replacement)** | Sources, locked format, no character limit, sale check, six-category set, line-4 hybrid table, link selection, coded compounds, de-domain, news posts, vendor code master, vendor-own-offer conflicts |
...
| PP_INBOUND_VENDORS | **v6 ← NEW** | Gate F added — sale-mechanics intake questions (stacking / boost-to-match) |
| ~~PP_INBOUND_VENDORS v5~~ | The four-month auto-reply outage, intake gates, the nine owed replies, paid-placement policy |
...
| **HR_SOCIAL** | **v19 ← NEW** | Format authority. **Templates now carry the photo-prep contract (`951f210`)**; two conventions still template-homeless (min resolution, export/delivery) — prompts carry them |
| **HR_INSTAGRAM_PROCESS** | **v3 ← NEW** | The how-to — now seven recorded failures. #7: the rule with no written home. The read attaches to the deliverable, not the request phrasing |
...
**⚠️ PP_X_POSTS v56, v50 and the v47/v48/sale-post addenda are SUPERSEDED.** v51 is a full replacement and stands alone. Do not read them as supplements.

---

### 🆕 NEW DOCS (Aug 29, 2026)
| Doc | Covers |
|---|---|
| **PP_AI_CHAT v1** | The chat assistant: architecture, the locked guardrails (dosing / codes-prices-stock / injection), the corpus + its build guard, retrieval scoring, the UI, "Powered by Claude" attribution rules, and the transformation-layer bug class |
| **PP_SOCIAL v1** | Instagram + Facebook. Which tool makes what (CC owns news carousels, CD owns visual composition), the carousel pipeline, and the three-surface news pattern. ⚠️ NOT the same as HR_SOCIAL — do not cross-apply |
...
**X posts** → **PP_X_POSTS v56 (this doc alone)** · PP's live `/coupons/<slug>` for code, catalog, line 4 · the vendor's own site for stock and the sale check
...
**Reveal gate / anti-attribution** → PP_SNIPPET_INVESTIGATION v2 · PP_COMMISSIONS v9 · `src/data/reveal-gate-vendors.ts` · PP_X_POSTS v56 §11 *(the posting view)*

**Inbound vendors** → PP_INBOUND_VENDORS v5

**Coupon page structure** → PP_VENDOR_PAGES v4 · PP_SITE_DESIGN v4

**HR social / Instagram** → **HR_SOCIAL v19** (authority) · **HR_INSTAGRAM_PROCESS v3** (how-to) · HR_PROGRAM_EXPORT · SC_FORM_CUES_EXPORT *(strength days only)*
...
### 🆕 Added Aug 28–29, 2026

**Codes render UPPERCASE on PP** regardless of portal display. `check:codes`' CODE_RE is uppercase-anchored, so **a lowercase code is an unguarded code** (proven: uppercasing OMP + Athena raised the guard count 145 → 152).

**A portal saying "change your code" may change the LINK instead.** Swiss Chems: the setting moved the referral link, not the coupon. **Verify which field actually moved before touching the repo.**

**Before changing a code string anywhere, inventory EVERY occurrence with its owning vendor.** `PROF10` is shared by three vendors; `?ref=PROF15` at aero-peptides is an affiliate *identifier*, not a coupon. A blind global replace breaks live vendors.

**FAQ content hides in a different shape on every page type** — now a **four-exhibit** blind spot: igf-1-lr3's `faqs` array ×2, bpc-157's accordion, and the 54 coupon pages (inline `<FAQItem>` JSX with titles from `buildCouponMetadata()` in a separate layout file, so they extracted with empty titles and zero FAQs). **Any sweep over page content must handle all of these shapes.**

**Rationale lives as a comment at the point of use.** Design reasoning that isn't obvious from the code goes beside the code — it survived a compaction with nothing needing re-derivation. Memory holds session state; ref docs hold cross-workstream strategy; **the code holds why the code is like that.**

**The GSC daily indexing quota FLOATS** — 11 accepted Aug 27, 9 accepted Aug 28. Never encode a count in a prompt; always "continue until GSC refuses, report where it stopped."

**Transformation layers need a contract about what they receive.** Four bugs in one build came from steps that didn't know their input's shape (a case-insensitive regex eating the word "profile", a strip-pass running after substitution, a field that already contained its own suffix, a scoring signal that never fired). **Where a prompt instruction is a request, a code-level pass is the guarantee.**

**CC's default cwd is the APP repo** (`profpeptide-app`), not the site (`~/dev/profpeptide`). Known harness behavior; gate accordingly.

**HR ref docs are not reachable from CC's environment** — the newest on disk is HR_SOCIAL **v9** while the governing version is **v19**. Any HR prompt saying "read §N of v19" is unexecutable, and a stale v9 propagated a wrong number into a template. **Decision owed: commit HR docs into the repo.**


**HR templates are the CD build contract as of `951f210` (2026-08-26):** photo-prep (grayscale-first, resize order, verbatim placeholder, acceptance check) is IN the template files. **Any rule CD must follow lives in the templates — chat context is a cache, not a home.** Two conventions await their template pass: minimum source resolution ≥1080×1350, and export structure/delivery scope — until then every CD prompt states them. HR posted through W8D6 (built + verified; posting Mark-side). Next: W9D1, APEX.

**FAQ section state (2026-08-25):** nine question pages LIVE at `f4fd5c4` — needle pilot, insulin needles, bac water, and six compound dosing pages (reta/tirz/sema cited-protocol; CJC-Ipa frequency-framed; BPC-157/TB-500 honest no-protocol). PP_FAQ v7 §4 is the state table. **Next: side-effects pilot (thymosin beta-4).** Standing rules now in force: profile "Common question →" links · pinned Where-to-Buy trio on every compound FAQ page · citations decide the page shape.

**Profile audit QUEUED (Mark, 2026-08-25):** CC may audit peptide profiles for content and consistency. Known defect driving it: **BPC-157 and TB-500 profiles carry entirely uncited dosing sections** (convention figures, Huberman protocol, mcg/kg conversions — no citation markers). Scope in PP_FAQ v7 §5.

**Hydro Research = inbound applicant #10** — recon complete (Gate B PASS with live Janoshik portal verification; C PASS via Store API; D weak, no legal entity; A unknown, self-hosted affiliate, lifetime-attribution claim unverified). Reply drafted with Gate D/A/F questions + the H-(…) decode ask. PP_INBOUND_VENDORS v6 Gate F (sale stacking / boost-to-match) applies to all future intakes.

**The FAQ project (PP_FAQ v7 §1) — do not lose this framing again:** people google specific questions; PP answers *that question* on one page, comprehensively, so they don't have to dig through Reddit — then links to the canonical page for depth. Reported-trial-dosing questions ("how often do you take reta") are IN scope; personal advice is not. **Cannibalization with profiles is a non-concern — Mark, explicitly: the profiles aren't getting clicks anyway.** This was re-explained multiple times before being recorded; it is now recorded. Nav label **FAQ**, hub is the existing /faq page, routes are `/faq/[slug]` in question form.
...
**Aero's code is PROFPEPTIDE, not PROFPEPTIDE15.** PP_X_POSTS v56 and earlier were wrong; vendors.ts and every live surface carry PROFPEPTIDE. The `?ref=PROF15` in Aero's URL is a GoAffPro affiliate identifier, not the coupon code.
...
**🔴 The affiliate platform map is STRANDED.** PP_COMMISSIONS_v6_ADDENDUM_platform_map was written to be inserted as §7 of PP_COMMISSIONS. **It never was — v9 carries no platform content.** The only vendor→platform mapping lives in that unmerged addendum, and it self-describes as incomplete and partly inferred. ⚠️ It also **contradicts PP_VENDOR_PROSPECTING v34 §8**, which marks Post Affiliate Pro a dead end while the addendum identifies PAP as the platform behind the Spartan/Almighty/Ignite binding failures. Unresolved. Fold into PP_COMMISSIONS v10 as a real section.
...
**v17 (August 26, 2026):** **HR_SOCIAL v19** + **HR_INSTAGRAM_PROCESS v3** — the W8D6 arc: failure #7 (the rule with no written home), the template contract at `951f210`, the prompt-drafting rule, two conventions queued for templates. Image-fit question closed.

**v16 (August 25, 2026):** **PP_FAQ v7** (state, evidence split, standing rules). **PP_X_POSTS v56** (EZ sale example expired; /prices OG outage recorded; FAQ pages linkable). **PP_INBOUND_VENDORS v6** (Gate F). Standing corrections: FAQ state · the queued profile audit · Hydro as inbound #10.

**v15 (August 24, 2026):** **PP_FAQ v7** created as the governing doc for the FAQ section — Mark's framing crystallized after repeated re-explanation. PP_ANSWERS demoted to research record. Standing correction added so the framing survives session boundaries.
