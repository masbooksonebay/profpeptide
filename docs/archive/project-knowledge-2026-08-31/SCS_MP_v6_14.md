...
**Version:** v6.14 (July 14, 2026) — adds: never-tier-on-a-proxy-metric (rank ≠ traffic); "classes correct ≠ behavior correct"; aesthetic changes need eyes; Claude must not filter silently; conflicted ratings are not a gate (+ no user reviews on PP); three-tier page division; PP↔Capstone conflict of interest; design sandbox = committed noindexed route; brand mark = SVG. Carries v6.13 forward (App-repo git-backup rule) (push after app work; EAS builds from local git and never forces a push, unlike auto-deploying sites) and the **Affiliate-attribution verification rule** (confirm BOTH code + link paths actually track before investing promotion/a page). Carries v6.12 forward unchanged.
...
## 🆕 v6.14 RULES — added July 14, 2026

### 📏 RULE #0 COROLLARY — NEVER TIER ON A PROXY METRIC
Similarweb **global rank is NOT a traffic proxy.** Proven wrong hard this session:
- Biolongevity: rank #843K → **916K actual monthly visits** (we'd tiered it "smaller")
- Prime Peptides: rank "undefined" → **484K ↑25%**
- Mile High: rank #499K → 160K
Rank and visits measure different things and correlate poorly at this scale. **Pull the real metric or say "unverified" — never tier, rank, or decide on a proxy.** Generalizes: if the metric you have isn't the metric the decision needs, get the right one.

### ⭐ "CLASSES CORRECT" ≠ "BEHAVIOR CORRECT" — VERIFY BEHAVIOR, NOT MARKUP
CC verified the sticky TOC structurally (`sticky top-24` + `lg:items-start` + no `overflow` ancestor) and reported it working. **It wasn't** — the TOC had 14 items and was taller than the viewport, and per spec an over-tall sticky element scrolls until its bottom edge then pins. Markup correct, behavior broken.
**Rule: for anything visual or interactive, the scroll/click/hover test is the only real check.** A clean `tsc`, a clean build, and correct classes prove none of it. Every CC prompt touching UI behavior must say: *"actually scroll/click-test this, don't just check the markup."*

### 👁️ AESTHETIC CHANGES REQUIRE EYES — BUILD-PASSES PROVE NOTHING
The light-mode refinement compiled, verified in compiled CSS, and was *invisible* (#FAFAFA vs #FFFFFF = 5 points). Mark caught it only by looking. Then it turned out the homepage was the wrong page to judge on (no cards → the off-white + card-float had nothing to act on).
**Rules:** (a) never deploy an aesthetic change sight-unseen because the build is clean; (b) judge a change on the page type it actually affects; (c) "subtle" that's *imperceptible* is the same as not shipping it — dial to clearly-visible-but-tasteful.

### 🚫 CLAUDE MUST NOT FILTER SILENTLY
Three times this session Claude quietly trimmed a list before handing it over (33 of 51 vendors; 25 of ~70; dropped Amino Club from a top-10 because its figure was contested) without saying a filter had been applied. Mark caught each.
**Rule: state the filter, or don't filter.** If a list is being narrowed, say what was removed and why, in the same message. When a value is contested, **include it flagged** — don't drop it.

### 📊 CONFLICTED RATINGS ARE NOT A GATE
PeptiPrices and PeptideCritic rate the same vendors incompatibly (Modern Aminos 4.9/1569 vs 3.6/10; Atomik 2.9 vs 4.5; Soma 3.3 vs 5.0). Tiny samples, self-hosted, and **both platforms have affiliate relationships with the vendors they rate.** Claude built an exclude-list on this data and had to retract it.
**Rule: user ratings from affiliate-conflicted platforms = weak tiebreaker, never a gate.** Verifiable signals only (published COAs, batch numbers, commission terms, traffic).
**Corollary — PP will NOT add user reviews.** Same conflict (PP earns from the vendors it would rate), guaranteed astroturfing in a 137-vendor market, cold-start with 0–2 reviews per vendor, and real product cost. PP's accountability mechanism is the **Vendor COA & Testing-Transparency Index** — objective, verifiable, unfakeable, and the actual moat.

### 🏛️ THREE-TIER PAGE DIVISION (LOCKED) — protects the brand as the roster scales
As PP's affiliate roster grows 23 → 60+, "curated directory of trusted vendors" silently becomes "everyone who'd have us." Independently validated: **PeptideCritic lists 137 vendors and monetizes ~31 (23%)**, with an explicit disclosure link.
- **/prices** — comprehensive, everyone, reporting function, NO quality claim
- **/coupons** — every vendor we have a code for; reframe from "curated directory of trusted vendors" → "codes we have"
- **/best-peptide-vendors** — stays genuinely curated, quality bar intact, small
⚠️ **The /coupons language change is still OUTSTANDING and gets harder the longer it waits.**

### ⚠️ PP ↔ CAPSTONE CONFLICT OF INTEREST (unresolved — decide deliberately)
Mark owns **both** Prof. Peptide (independent comparison/review site, whose coupon page states *"Affiliate relationships never determine vendor inclusion, ranking, verification, or our editorial conclusions"*) **and** Capstone Peptides (a vendor). Listing or ranking Capstone on PP without prominent common-ownership disclosure is the exact credibility failure PP is built to avoid — and precisely what PeptideCritic gets criticized for (own store alongside reviews).
Options: exclude Capstone from PP; include with explicit "same ownership" labeling; or keep brands publicly separate and accept the link is traceable. **Decide in advance, not when asked.**
Same logic killed **paid placement** on PP for now: PeptideCritic's ★Premium Partner is paid placement, but it directly contradicts PP's independence claim. A free "partner with us" inbound page is fine; paid placement needs the brand-language change made deliberately first — and PP has no traffic leverage yet regardless.

### 🧪 DESIGN SANDBOX = A REAL COMMITTED ROUTE, NOT AN UNCOMMITTED DIFF
The old "bpc-157 prototype" was an **uncommitted working-tree diff on the live page** plus an untracked component — which forced a standing "exclude from commits" rule that tangled three separate prompts and blocked CC from touching the very file it was layered on. Replaced with **`/prototype/profile`** and **`/prototype/logo`**: real routes, committed, noindexed, sitemap-excluded, unlinked, with live option toggles.
**Rule: design sandboxes are committed noindexed routes. Never an uncommitted diff on a live page.**

### 🎨 A BRAND MARK IS AN SVG, NOT CSS TEXT
The "Pp" tile rendered as CSS text in a div overflowed its box (48px glyphs + `leading-none` in a 56px tile). More CSS tuning is the wrong fix. **Letterforms as SVG paths** = no font dependency, crisp at any size, recolorable for dark mode, and the same asset seeds the favicon. Wordmark stays CSS text (selectable + accessible).
Tooling note: **CD could not produce it** (returned an off-brief periodic-table tile); **CC has no image tools**. **ChatGPT is the image-gen tool for icons** (Mark's standing view, now confirmed twice).
