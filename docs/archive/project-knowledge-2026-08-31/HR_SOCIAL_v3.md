# HR Social — Promotion Plan (IG + FB + X) — v3
...
**Version:** v3 (July 1, 2026) — adds the **per-post content spec** (the missing SCS 19 decision Mark flagged: daily serialized "Day X · Week Y" posts; the carousel slide-by-slide; "Day X · Week Y" is a header not the whole card; caption hook-first; B&W free-stock action background; both offers reconciled). v2 added the 2026 research + locked the first-100 offer + expanded to IG+FB+X. v1 consolidated strategy from SCS 15/18/19 and corrected the font to Barlow Condensed Bold.
...
**The carousel (IG) — slide by slide:**
- **Slide 1 — COVER (stops the scroll, earns the read):** the workout card. **"Day X · Week Y" as a HEADER on the card, not the whole card** (a bare "Day 1 · Week 1" label gives a scroller nothing to stop for — explicitly rejected in SCS 19). Workout name, HR wordmark (Barlow Condensed Bold, `#FFED00`), over a **B&W functional-fitness/HYROX action background** (sourced from free / non-copyrighted stock). The workout itself is *on* the image — they see the real session, that's the value, they stop + save + read.
...
- **Slides 3–4 — COACHING:** pacing, scaling up/down, the common mistake — one idea per slide, value front-loaded (slide 3 gates the re-serve).
- **Final slide — APP PITCH + CTA:** "Tracking + AI coaching + the whole 12-week program in the app" · **first 100 downloaders get everything free for life** · "link in bio." Plus a comment-driving question.
