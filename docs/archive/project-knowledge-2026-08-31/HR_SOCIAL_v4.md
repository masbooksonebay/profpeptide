# HR Social — Promotion Plan (IG + FB + X) — v4
...
**Version:** v4 (July 1, 2026) — **locks the 5-slide carousel** (cover → workout → coaching·pacing → coaching·form/technique → app pitch), form/technique slide sourced from the app's existing technique content (doubles as an app preview). v3 added the per-post content spec (daily serialized "Day X · Week Y"; caption hook-first; B&W free-stock background; both offers reconciled); v2 added the 2026 research + first-100 offer + IG/FB/X scope; v1 consolidated strategy + corrected the font to Barlow Condensed Bold.
...
## 🖼️ Per-post content spec — image slides + caption (LOCKED, from SCS 19 + v2 research)

**The daily model:** post **1 workout per day**, **serialized** — "Day 1 of Week 1 of a 12-week program," then Day 2, Day 3… through the full program. Builds a follow-and-return habit (feeds both the algorithm and the download funnel). **The full 12-week program is given away** across the daily posts — fine because future programs mean it's not the only asset; the app (tracking + AI) is the upgrade the free workouts point to.

**The carousel (IG) — 5 slides (LOCKED):**
- **Slide 1 — COVER (stops the scroll, earns the swipe):** the workout card. **"Day X · Week Y" as a HEADER on the card, not the whole card** (a bare "Day 1 · Week 1" label gives a scroller nothing to stop for — explicitly rejected in SCS 19). Workout name, HR wordmark (Barlow Condensed Bold, `#FFED00`), over a **B&W functional-fitness/HYROX action background** (sourced from free / non-copyrighted stock). The workout itself is *on* the card — they see the real session, that's the value, they stop + save + read.
- **Slide 2 — THE WORKOUT:** the full session laid out clean (movements, reps, distances, rounds) in a legible body font (NOT condensed-bold).
- **Slide 3 — COACHING · pacing/strategy:** how to run the session — pacing, effort targets, where people blow up, scaling up/down. (Slide 3 gates the re-serve — real value here pushes viewers deeper and triggers the algorithm's second distribution pass.)
- **Slide 4 — COACHING · form/technique:** movement cues + common form mistakes, **sourced from the app's existing technique content.** This slide doubles as an authentic app preview — it shows the coaching depth that downloading unlocks.
- **Slide 5 — APP PITCH + CTA:** "Tracking + AI coaching + the whole 12-week program in the app" · **first 100 downloaders get everything free for life** · "link in bio" · plus a comment-driving question.

**Why 5 (not 3):** the 2026 algorithm gates a carousel's 24–48h **re-serve** (a second free distribution pass) on viewers reaching **slide 3+** — so putting real coaching on slides 3–4 (not the app pitch) is what trips the re-serve and pushes the "sell" to the end where it's earned. Chosen because there's genuine coaching + form/technique content to fill it (repurposed from the app), NOT to pad a number — thin coaching slides would stall viewers and hurt. *(If a given workout is self-explanatory and lacks the depth for two coaching slides, collapse to 4 or 3 rather than pad.)*

> **Source note (RULE #0):** the actual per-workout form/technique copy comes from the app's own technique content — pull it via the program export (`HR_PROGRAM_EXPORT.md`), don't invent cues. This spec defines the *structure*; the real content fills it once exported.

**The caption:**
- **First line = a HOOK, never "Day 1 of Week 1"** (IG cuts captions after ~1–2 lines behind "…more"; that info is already on the image, and it's boring). Lead with the hook.
- Then: the workout details/notes → the app pitch (tracking + AI) → "link in bio."
- **No clickable caption links** — the single bio link carries all traffic.

**The two offers (complementary, not competing):**
- **Free 12-week program** = the daily reach/habit engine (top-of-funnel).
- **First 100 = everything free for life** = the install hook / CTA.
- Both point to the same action: download the app. Daily-post final slide + caption CTA can carry the first-100 offer; the pinned explainer post frames both.

**Bio link:** App Store, or a simple landing page with the App Store button *plus* the free program (catches the "just want the free workouts" crowd and pitches the app to them).
...
## 📅 Changelog
**v4 (July 1, 2026):** **Locked the 5-slide carousel** — cover → workout → coaching·pacing/strategy → coaching·form/technique → app pitch/CTA. Form/technique slide sourced from the app's existing technique content (doubles as an authentic app preview). Chose 5 (over 3) because there's real coaching + form/technique content to fill it (repurposed from the app), not to pad — with a collapse-to-4/3 allowance for self-explanatory workouts. Re-serve rationale recorded (slides 3–4 coaching trip the slide-3 re-serve; pitch earned at the end).
**v3 (July 1, 2026):** Added the **per-post content spec** (the piece Mark flagged as missing from the doc, decided in SCS 19): daily serialized "Day X · Week Y of a 12-week program" posts; the full 12-week program given away across daily posts (fine — future programs mean it's not the only asset); carousel slide-by-slide (cover with "Day X · Week Y" as a header not the whole card + B&W free-stock action background → workout → coaching → app-pitch/CTA); caption hook-first (never "Day 1 Week 1"); both offers reconciled (free program = reach engine, first-100-free-for-life = install hook); bio-link landing-page note.
