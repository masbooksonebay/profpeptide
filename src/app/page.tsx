...
// the active (non-retired) vendor list, compounds-compared from the price index. Describes the
...
// Compounds a visitor can actually comparison-shop (≥3 vendors → an indexable /prices/<slug>
// page). Derived via indexablePriceCount() so there's ONE definition of "indexable". This is a
// count of COMPOUNDS, not price rows (~1,580) — hence the "Compounds compared" label below.
const compoundsCompared = indexablePriceCount();

const trust = [
  { value: `${profileCount}`, label: "Peptide profiles" }, // derived /peptides route count
  { value: `${compoundsCompared}`, label: "Compounds compared" }, // indexable /prices/* compounds
