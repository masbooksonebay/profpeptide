...
export default function SwissChemsCouponPage() {
  const v = vendors["swiss-chems"];
  const rows = vendorProductRows("swiss-chems");
  const discountPct = vendorDiscountPct("swiss-chems");
  const autoApply = codeAutoApplies("swiss-chems");
  const shopUrl = makeShopUrlFor("swiss-chems");
  return (
    <div className="section max-w-3xl">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-1">
        <h1 className="text-3xl font-bold text-[#16181B] dark:text-slate-100">Swiss Chems Discount Code: PROFPEPTIDE &mdash; Save {discountPct}%</h1>
      </div>
      ...
      <div className="space-y-8">
        <div>
          ...
          <dl className="space-y-2 mb-4">
            ...
            <div className="flex flex-col sm:flex-row sm:gap-3 text-sm">
              ...
              <dd className="text-gray-600 dark:text-slate-300">
                <Link href="/peptides/epitalon" className="text-[#3A759F] hover:underline">Epitalon</Link>, <Link href="/peptides/glutathione" className="text-[#3A759F] hover:underline">Glutathione</Link>
              </dd>
            </div>
            <div className="flex flex-col sm:flex-row sm:gap-3 text-sm">
        ...
        <div>
          <h2 className="text-lg font-semibold text-[#16181B] dark:text-slate-100 mb-5">Swiss Chems catalog &amp; prices</h2>

          <VendorProductGrid rows={rows} discountPct={discountPct} shopUrlFor={shopUrl} />

          <p className="text-xs text-gray-400 dark:text-slate-500 mt-3">
            Struck-through prices are Swiss Chems&apos; list price; the bold figure is{" "}
            {autoApply ? (
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <FAQItem
              ...
              a={`Yes. Use code PROFPEPTIDE at checkout to save ${discountPct}% on your Swiss Chems order. This code is verified and maintained by Prof. Peptide.`}
            />
            <FAQItem
              q="How do I use the Swiss Chems discount code?"
              a="Add your items to cart at swisschems.is, proceed to checkout, and enter PROFPEPTIDE in the coupon field. The discount is applied to your order total."
            />
            <FAQItem
